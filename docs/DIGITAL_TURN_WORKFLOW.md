...
**Resident handoff:** a clean print/PDF titled "Your Home Preparation Checklist", with property, unit, review date, recorded checks, applicable/not-applicable results, and named tech/reviewer sign-offs. Generate it from an immutable finalized revision, not the current mutable board fields. Keep the internal parts list, costs, vendor discussions, damage/charge notes and prior-resident details out of it. Per the updated request, include mailbox number, key/fob/remote counts, parking/garage assignment and optional resident-authorized door/building/gate codes. Never include staff/master/maintenance codes. Code inclusion must be explicit and restricted to the authorized household's handoff, excluded from general reports, logs and exports; revocation/reissue and secure storage require implementation before live use.

PREVIEW-004 expands the sample into two printed columns with 45 source-derived checks across general/HVAC, kitchen, bathrooms, living areas/bedrooms, technician safety/electrical and independent final review. Provided internet and valet trash have separate results. It remains one Letter page, with a three-column access section and both sign-offs. Re-read the original workstation PDF on 2026-09-07; its second-page parts/rework sections remain internal. This is a fictional layout only, not new persisted access fields or a live generator. Chromium PDF page count and mobile overflow were checked. Production output still needs variable-content fit checks and must never truncate unresolved findings.
...
Resident format requirement (2026-09-07): keep the handoff to one Letter page. Use grouped areas such as Bedrooms checked, Bathrooms checked, Kitchen, Living areas/entry and Outdoor areas, not numbered rooms. Room counts do not add report rows. A grouped result covers every applicable check in that category; unresolved checks must prevent a passed group, and non-applicable areas must be identified honestly. Retain detailed room/device evidence internally, separate technician and reviewer sign-offs, both property/company identities and resident-facing follow-up. The future generator must validate layout with actual names, logos and notes, never clip text or hide unresolved work to force a page fit. PREVIEW-003 was rendered with Chromium and verified with pdfinfo as one Letter page; the printed page was visually inspected.
...
- [ ] Resident handoff: finalized printable/PDF checklist, separate draft/internal output, page-width/page-break testing, escaped content, excluded internal/master access data, explicit household-only access inclusion, and revision history.
