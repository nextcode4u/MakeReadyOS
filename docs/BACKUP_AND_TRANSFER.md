...
- Project attachment file bytes are excluded; project records, categories, comments, tasks, wiki references, map coordinates, and attachment metadata are included. Each project now also includes `quotes` and `costLines`, preserving integer-cent amounts, versions and timestamps; attachment `quoteId` preserves the quote association. Older backups default those additions to empty/null. Merge retains existing entries rather than overwriting newer costs and rejects cross-project ID collisions. Keep the separate uploads backup for PDF/image bytes. See the partial-transfer limitations above.
