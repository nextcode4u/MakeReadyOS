...
- The production web build also separates React, React Query, and other vendor modules into explicit Vite chunks. The main app chunk-size advisory has returned in the September 23 build; inspect current bundle composition while preserving the table-first startup shell.
...
Future scale work may add cursor-based continuation for the primary board frontend, generated JSON shadow columns for frequently filtered custom-field types if real datasets require it, incremental drawer-history controls, report aggregation, generated OpenAPI route schemas beyond the current static baseline, and measured virtualization without changing stable field keys. Application webhook event queueing is already implemented; expand coverage only where needed. Investigate the current Vite chunk warning and split appropriate shared dependencies before increasing warning thresholds.
