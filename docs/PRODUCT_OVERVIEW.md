...
The PM module includes dashboard, calendar, task, template, history, and report views. Completing or skipping a task generates the next occurrence from the template so recurring work continues without a separate dispatch engine, and PM overview/task/calendar reads also ensure one open task exists for every active template. PM templates and tasks can also attach Property Wiki references for SOPs, equipment notes, known issues, and emergency context. Native transfer now includes PM templates, tasks, attachment metadata, and PM Wiki references, while uploaded file bytes still follow the standard upload-backup path. PM exports now include direct PDF output in addition to CSV and Excel-compatible formats. The current design intentionally avoids approval workflows, work-order accounting, vendor dispatch complexity, and a dedicated background scheduler.
...
Marker color for unit pins can still follow risk level, vacancy status, board section, assigned tech, or make-ready status, and the unit directory keeps unmapped units visible so setup gaps are clear. Property Maps remain local and self-hosted rather than GIS/CAD based. Native backup includes map metadata and marker locations; uploaded map/image bytes still rely on the normal upload-volume backup path.
...
Setup also manages property-scoped board sections/tables, standard colored board options, and reusable floor plans. Board sections now have a dedicated manager separate from Board Labels so operators can rename `Ready Units`, `Make Ready`, `Down Units`, and `Archive` without mixing workflow-section configuration with status-option maintenance. Floor-plan cells and the item inspector select managed plans rather than editing uncontrolled text; selection synchronizes linked unit beds, baths, square footage, and description context. Existing freeform values remain explicitly marked `LEGACY` until mapped. The table surface includes a fast `+ Add item` row at each group: entering a unit number creates the turn in that section and creates the unit if needed. Protected bulk actions now include moving selected turns between board sections and selecting an active staff assignee rather than typing a name.
