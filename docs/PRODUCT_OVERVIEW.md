...
- lease-compliance issue tracking for patios, balconies, exterior conditions, notices, recurring concerns, and evidence history without turning MakeReadyOS into a legal-notice system
...
Inspection evidence intentionally stays attached to the make-ready turn record rather than a separate persisted inspection entity. That keeps wiki references, charge evidence, checklist execution, comments, vendors, and final historical review anchored to one turn-level operational record.
...
The Activity workspace also includes a daily manager report for operational handoff. It summarizes what changed on a selected day, including turns marked ready, availability imports, created/updated turns, archive and restore actions, and exception/risk events. The summary cards and detailed row list both surface those lifecycle changes, and the report includes property/unit context plus CSV export so managers can reconcile MakeReadyOS with external property systems without using human notes as import history.
...
Unit history is available from the item drawer and unit-history API by deriving a timeline from make-ready lifecycle dates, audit/activity records, comments, attachments, checklist completions, vendor assignments, automation runs, and current risk state. Turn history is derived from make-ready records linked to the unit, so prior turns can be summarized without duplicating operational source data. The Setup workspace Archive / Occupied Units panel exposes active/archive/all turn views plus read-only occupied directory visibility with search, turn counts, key dates, direct item-detail links for post-move-in lookup, and explicit archive-history filters for unit, applicant-if-stored, turn reason/availability, inspection stage, and date windows when teams need to find recurring repair or damage patterns. It now also includes a dedicated unit-history inspector so staff can review one unit’s current turn, prior turns, dates, risk/checklist/vendor rollups, and fuller event timeline without depending on the drawer’s shorter history slice. New-turn creation now also refuses duplicate active turns on the same unit until the current live turn is explicitly opened, archived, or restored, which keeps archived history distinct instead of letting staff accidentally layer multiple live records over one unit. A script-friendly snapshot runner records property/day rollups for average days vacant, completed turns, overdue work, high-risk counts, and move-in windows.

## Workload Planning

The Planning workspace gives supervisors a lightweight weekly view of in-house work assignments, scheduled coverage days, unplanned active turns, and move-ins that are not covered by scheduled work. Managers and administrators can create and replan work blocks by item, staff member, category, date, and notes. Hour estimates are intentionally hidden because emergencies, parts, and vendor timing make exact capacity math unreliable. The main assignment list now separates active planning blocks from done/canceled blocks so daily planning review stays focused while closed work remains visible for context. My Work includes active planning blocks for field users, and the item drawer shows an in-house planning summary next to vendor work.
...
The vendor foundation tracks active/archived vendors, trade/category, contact information, preferred status, property service areas, basic insurance/license expiration dates, and assignments linked to make-ready items. The Vendors workspace now separates active and archived directory records so archive behaves like a recoverable state instead of feeling like delete, while assignment pickers continue using active vendors only. The most visible quick-add, directory, and assignment controls now also keep Spanish-safe labels and placeholders instead of dropping back to English during setup. Vendor assignments carry scheduled/due dates, status, notes, optional cost/reference fields, and are visible from the Vendors workspace and item drawer. Vendor dates can be shown as Schedule tracks; Dashboard and Risk include vendor overdue/follow-up/blocked-work signals. Native backup includes vendor records and assignments, but MakeReadyOS does not include accounting, payments, vendor portals, email, or SMS.

## Refrigerant Tracking

The Refrigerant workspace gives maintenance teams a simple operational log for refrigerant types, virgin cylinders, clean recovery tanks, dirty recovery tanks, unit charge/recovery events, and final recovery from empty virgin cylinders. Weight math is calculated from start/end weights, recovery tanks show 80/90/95 percent capacity warnings, and repeated additions on the same unit/refrigerant create manager-review leak flags. Admins manage refrigerant types; managers and techs can log field work; viewers can read scoped records; leasing and cleaning roles do not see the workspace. Refrigerant type setup now separates active and inactive types so older refrigerants remain available for history without cluttering new capture flows, and tank workspaces now do the same for active versus archived cylinders so retired tanks stay restorable without reading like live capture inventory. Native backup includes refrigerant configuration and logs, while CSV, Excel-compatible, printable HTML, and PDF exports support spreadsheet review and compliance handoff.

## Pool And Spa Logs

The Pool Log workspace gives properties a lightweight daily water-feature log for pools, spas, wading pools, splash pads, and other water features. Admins/managers configure pools/spas and chemical libraries; techs can record daily chemistry readings, operational checks, safety checklist values, notes, chemical additions, and pool-specific photos/PDFs. Pool setup now keeps active and archived facilities/chemicals visibly separate so archive no longer looks like delete; archived records can be restored from setup when a pool, spa, or chemical returns to service. The overview highlights pools not logged today, failed safety checks, out-of-range chemistry, and missing dosage inputs such as capacity or chemical concentration. CSV export and direct PDF reporting support review; missing logs and review-needed entries create deduped in-app notifications. External probes, weather feeds, email/SMS, and health-department integrations remain future work.
...
The PM module includes dashboard, calendar, task, template, history, and report views. Completing or skipping a task generates the next occurrence from the template so recurring work continues without a separate dispatch engine, and PM overview/task/calendar reads also ensure one open task exists for every active template. Template setup now separates active and archived PM templates so old recurring programs stay recoverable instead of silently disappearing from configuration. PM templates and tasks can also attach Property Wiki references for SOPs, equipment notes, known issues, and emergency context. Native transfer now includes PM templates, tasks, attachment metadata, and PM Wiki references, while full file continuity rides on the standard `backup-all.sh` database-plus-uploads snapshot path. PM exports now include direct PDF output in addition to CSV and Excel-compatible formats. The current design intentionally avoids approval workflows, work-order accounting, vendor dispatch complexity, and a dedicated background scheduler.

## Projects And Recommendations

The Projects workspace gives property teams a lightweight operational layer for recommendations, approved projects, simple tasks, comments, bids/quote tracking context, and location-aware follow-up without turning MakeReadyOS into a full capital-project or accounting platform. Admins and managers can create and manage scoped records; techs can participate in operational execution; leasing and viewer roles can read scoped data; cleaners are currently excluded from the workspace.

The current foundation includes dashboard, project list, recommendation list, map view, archive, and export/report surfaces plus attachments, record comments, simple project tasks, recommendation-to-project conversion, source tracking, deferred-maintenance planning fields, budget-year grouping, estimated-vs-actual cost fields, categorized project photo/file types, photo captions, multi-photo uploads, mobile camera-friendly upload flows, simplified quick-create modes, richer visual cards, recommendation aging visibility, lifecycle history, single-record PDF reports, richer multi-project reports, and My Work integration for assigned records. Category administration now separates active and inactive categories so deactivated labels remain recoverable without reading like current workflow choices, and the category-admin controls keep Spanish-safe labels/prompts instead of falling back to English during setup. The current implementation intentionally keeps bidding depth, map editing, create-from-anywhere integrations, and deeper cross-module integrations lightweight until real property usage validates the next step.

## Pest Control

The Pest Control workspace gives leasing, management, and maintenance a lightweight operational log for pest requests, vendor treatments, follow ups, make-ready pest needs, and recurring unit pest history without turning MakeReadyOS into a resident work-order or pest-management platform. Admins, managers, techs, and leasing can create, edit, close, archive, and note pest requests; cleaners and viewers can read scoped records. Fast-entry defaults keep the primary workflow close to the old Monday board: one unit or area, one pest request, minimal required fields, direct note/close actions, and direct create/open pest handoffs from the make-ready board and item drawer when a turn needs pest follow-up. Vendor administration now separates active and inactive contractors so assignment pickers only surface live vendors while older contractor records remain restorable for history.

The current foundation includes dashboard, active, make-ready, vendors, archive, and reports views plus quick-add creation, simple vendor defaults, searchable timestamped notes, photo/PDF uploads, recurring issue detection, make-ready linkage, CSV/PDF exports, and My Work surfacing for assigned requests. Active and make-ready request review now stays condensed by default: each pest issue opens from a compact summary row into a tighter action/detail shell instead of expanding into a mostly empty full-width sheet. The current implementation intentionally keeps board-level create/link actions, backup portability, deeper report variants, and Excel/OpenAPI coverage as follow-up work until real property usage validates the next step.
...
The workspace now also includes per-user favorites, recently viewed records, a lightweight record-detail panel with related content and audit-based change history, a dedicated Emergency Mode surface for critical records, workflow references/smart suggestions inside Make Ready, Preventive Maintenance, Pool Log, and Refrigerant, plus a Dashboard widget for emergency and pinned knowledge visibility. Active and archived/inactive knowledge records are now grouped separately inside the working lists for section records and wiki vendors so restore remains obvious without cluttering the default operational views. Native transfer now includes Property Wiki records, emergency/building metadata, related-entry/vendor links, and workflow-reference metadata, while per-user favorites/recent views remain local to the instance. The current foundation still intentionally avoids nested page trees, approvals, QR systems, or asset-management complexity.


## Unit Directory And Occupancy

MakeReadyOS now separates permanent unit inventory from active make-ready turns. Properties can store an occupancy goal percentage, and units can carry directory-level occupancy states such as occupied, vacant not leased ready, vacant not leased not ready, NTV not leased, NTV leased, vacant leased ready, vacant leased not ready, down, model, and unknown. Building, area, and floor metadata support both simple unit-only properties and larger communities with irregular building numbering. The Setup workspace includes merge-style paste/file import for comma-delimited, semicolon-delimited, or tab-delimited unit directories and availability reports, with a preview before writing. Directory imports update inventory only, and the preview now shows every new unit plus every existing-unit field change in unit order before import instead of only aggregate counts. Availability imports update operational availability and create/update active make-ready table rows for non-occupied availability statuses. Sparse files are safe: a unit directory can contain only unit number, floor plan, and square footage, and missing columns will not wipe existing unit metadata during updates. Recurring availability imports now include an API-enforced stale-report failsafe: when the report would overwrite newer or more advanced local board changes, MakeReadyOS blocks the import and requires an explicit override instead of silently letting the report win. The browser importer now also includes quick starting presets for Full, RealPage-style, Yardi-style, and Compact availability shapes plus broader low-risk header aliases such as `Unit #`, `Avail Status`, `Ready Dt`, `Market Ready`, and `Future Resident`, while the unit-directory side now mirrors that with a Yardi-style preset, aliases such as `Unit #`, `Unit Type`, and `Occupancy Eligible`, preview-safe filtering of obvious blank-unit summary/header noise rows, and automatic semicolon separator detection for spreadsheet exports. The Dashboard shows occupancy percentage, goal, total units, occupied units, vacant ready stock, and availability-report counts. Completed turns still belong in each property Archive section for later history review after move-in.

## Property Maps And Unit Directory

The Maps workspace is now a shared operational map layer rather than only a unit-layout tool. Authorized users can upload local property map images/PDFs, manage multiple maps per property, place building/area/floor markers, place unit markers, add shared pins for utilities/equipment/issues/wiki references, and see project or recommendation pins flow through automatically when those records already carry map coordinates. Search can find units, areas, buildings, custom pins, and project markers within the selected property map, while Emergency Mode reduces the surface to critical pins when teams need fast shutoff or access information.

Marker color for unit pins can still follow risk level, vacancy status, board section, assigned tech, or make-ready status, and the unit directory keeps unmapped units visible so setup gaps are clear. Active and archived property maps are now also grouped separately in the map selector so retired layouts remain recoverable without reading like current live map choices, and the most visible map-setup and fallback labels now stay Spanish-safe instead of dropping back to English. Property Maps remain local and self-hosted rather than GIS/CAD based. Native backup includes map metadata and marker locations; uploaded map/image bytes still rely on the normal upload-volume backup path.

## Frog Pond Visualization

Frog Pond is a whimsical but data-driven view over the same permission-scoped make-ready records. Operators can choose what frogs represent, group by property/section/risk/tech, color frogs by risk/vacancy/make-ready/property, choose from committed pond backgrounds, cap visible frogs for performance, pause animation when needed, and open the shared item drawer from any item-backed frog. Frog sprite sheets are cropped to 32x32 frames rather than rendered as whole sheets, and the accessory sheets now activate from real operating signals such as ready units, major scope, critical risk, assigned work, and near-term move-ins instead of sitting as unused runtime art. The panel also includes quick operational playbook presets and per-group overflow cluster chips so supervisors can jump from condensed pond overflow back into the exact grouped slice. It is intentionally local-preference based at this foundation stage and does not replace the table or dashboard.
...
An operational template library reduces setup time for common workflows. Admins and managers can preview and install templates for overdue work, imminent move-ins, missing dates, schedule-review warnings, pest follow-up, flooring scheduling, weekend schedule guards, vendor lead-time reminders, scope-day planning, date-sequence review, daily schedule load review, in-house/vendor routing review, cleaner-assignment review, balanced tech-assignment review, ready-unit stock expectations, and major scope. Installed templates become regular editable rules and remain disabled unless the operator explicitly enables them. Templates that depend on a missing custom field show the setup step instead of silently installing incomplete logic. The automation engine now also supports a scheduled least-loaded staff assignment action with eligible role/user filters, planning-aware balancing, planned-day caps, deterministic tie-breaking, preview visibility, and assignment notifications, while the bundled assignment templates remain review-first until field validation is complete.

Operational Library packs broaden setup beyond single automation templates. A pack can bundle reusable fields, labels, checklists, schedule tracks, saved views, and disabled automations for categories such as Make Ready, Leasing, Cleaning, Flooring, Paint, Pest, Scheduling, Risk/SLA, Notifications, and Reporting. Bundled packs appear in the Automation workspace alongside rule templates, property templates, rules, and run history, with compact section navigation to keep the page readable as the library grows. The workspace now includes a guided bundled-vs-imported pack wizard with field/option mapping previews plus installed-pack detail inspection, so operators can review what a pack targets before install instead of working from a raw JSON box alone. Packs are previewed before install, duplicate-safe, and data-only; imported JavaScript is never accepted.

Property templates extend that setup model to full property configuration. Admins and managers can save board sections, option sets, custom fields, floor plans, schedule tracks, shared views, dashboard presets, checklist templates, and disabled automation rules from an existing property, then dry-run apply the template to another property. The most visible property-template and rule-builder controls now also keep Spanish-safe labels, descriptions, lifecycle wording, and trigger names instead of falling back to English in normal setup flows. Templates intentionally exclude live turns, residents, comments, attachments, users, tokens, sessions, and history.
...
Mobile operators can install MakeReadyOS from supported browsers for an app-like home-screen launch. The PWA layer now caches the static shell plus previously fetched JSON API GET responses with a network-first fallback, so recently viewed workspaces remain readable when signal drops without pretending fresh server data exists.

Weak-connection handling is now operational rather than banner-only. The web UI detects browser offline state and API reachability failures, shows queued-sync state in the connection banner, keeps staged photo/file blobs in IndexedDB until upload succeeds, and automatically replays supported mobile-field writes after reconnect. Current queue coverage includes Make Ready status patches and attachment uploads, Projects quick capture and attachment retries, Lease Compliance quick capture and photo uploads, Pest quick adds and attachment uploads, Pool Log entry creates and attachment uploads, and Preventive Maintenance complete/skip/upload actions. Remaining offline work is explicit conflict handling plus queue coverage for comments, checklist changes, and deeper edit flows.
...
Language preferences support English and Spanish for the core app shell, sign-in flow, and user-management workflow. Administrators can assign a default language per user, and users can switch their own language from the toolbar. The admin user directory now also separates active and inactive accounts during filtering so reactivation cleanup is easier to review without inactive staff reading like current live operators. This is the localization foundation; deeper operational copy can be translated incrementally without changing permissions or data bindings.
...
The old saved-view/filter sidebar has been removed from the primary board layout to reclaim horizontal working space. A narrow left module rail remains for MakeReadyOS plus active side workspaces such as Preventive Maintenance, Projects, Refrigerant, Pool Log, and Property Wiki. Small-screen users receive a compact board-card scan view with selectable items before the full spreadsheet grid, plus a phone-first board toolbar for visible-item selection, selected-item batch actions, and direct board setup shortcuts without depending on the desktop header. Kanban remains a horizontally scrollable workflow surface. The make-ready drawer now also gives operators a compact collaboration-at-a-glance layer: recent-update summary cards, pending-sync visibility, and grouped attachment review cards for pending uploads, classification work, charge candidates, and recent files before they drop into the full gallery/editor flows.
...
Setup also manages property-scoped board sections/tables, standard colored board options, reusable floor plans, and the core property/unit inventory behind the board. Board sections now have a dedicated manager separate from Board Labels so operators can rename `Ready Units`, `Make Ready`, `Down Units`, and `Archive` without mixing workflow-section configuration with status-option maintenance. Floor plans and schedule tracks now also separate active and archived records inside setup so retired configuration stays recoverable without reading like live defaults, and the property/unit inventory lists now do the same for archived properties and units. Floor-plan cells and the item inspector select managed plans rather than editing uncontrolled text; selection synchronizes linked unit beds, baths, square footage, and description context. Existing freeform values remain explicitly marked `LEGACY` until mapped. The table surface includes a fast `+ Add item` row at each group: entering a unit number creates the turn in that section and creates the unit if needed. Protected bulk actions now include moving selected turns between board sections and selecting an active staff assignee rather than typing a name.

## Schedule Planning

The schedule surface treats `NTV / Notice to Vacate` as the expected move-out notice track and `Vacated` as the vacancy date track. Built-in tracks also include Make Ready, Move-In, and Flooring. Setup lets managers/admins use presets for common tracks, then create, rename, enable/disable, archive/restore, reorder, filter, group, and set risk cues for tracks. The setup editor now adds source/color/grouping summaries, clearer warnings when another track already owns the same date source, and preserved labels when an older custom field dependency has since been archived so operators can still understand and repair the configuration instead of losing context. Color can derive from status, scope, a selected managed/custom option field, a fixed color, or a neutral fallback, and the legend follows the active configuration. Each active calendar panel also shows compact compatibility guidance for risk cues, weekend items, crowded days, high-risk work, and property operating-calendar conflicts in the current filtered month, while affected calendar days get visible badges for weekend, Monday/Friday, crowded-day, high-risk, and overdue conditions. Active custom date fields can supply Painting, Cleaning, Pest follow-up, vendor dates, inspections, or other property-specific schedule tracks without adding hardcoded modules.
...
Property Maps remain local and self-hosted. Unit markers still open the shared item drawer, while shared pins and project markers now provide a property-aware visual layer for emergency utilities, recurring issues, recommendations, and other location-based operational context without forcing a GIS-style system. Selected map units, areas, and shared pins can also hand off directly into prefilled Projects recommendation capture or Pest Control quick-add, and dense marker clusters spread slightly on the canvas so high-traffic locations remain readable.
