...
Partial-import and numeric-alias follow-ups above are now deployed as `c4b3ebb` (superseding their local-only notes). Five domain/permission tests and lint passed; partial import browser coverage passed in `logs/e2e-20260907-225434.txt`. API/web production builds succeeded. Database backup `makereadyos-db-20260907-230113.dump` and upload backup `makereadyos-uploads-20260907-230114.tgz` were verified; prior images retained as `rollback-mailbox-pre-c4b3ebb`. All services healthy, no pending migrations. Live read-only preview matched TA `11` to `011` and flagged an unknown row without changing mailbox data. Units UI, both property prompts, clipboard and mobile containment also passed.
