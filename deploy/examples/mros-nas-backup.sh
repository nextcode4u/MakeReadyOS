...
trap 'exit 143' TERM
trap 'exit 130' INT
findmnt -rn -M /mnt/app-backup -t cifs >/dev/null
[[ $(findmnt -rn -M /mnt/app-backup -t cifs -o SOURCE) == //NAS_HOST/SHARE ]] || { echo 'Unexpected NAS share'; exit 1; }
api=$(docker compose ps -q api)
[[ -n "$api" && $(docker inspect -f '{{.State.Paused}}' "$api") == false ]]
[[ $(docker inspect -f '{{range .Mounts}}{{if eq .Destination "/app/uploads"}}{{.Source}}{{end}}{{end}}' "$api") == /mnt/app-backup/uploads ]]
...
SQL
timeout 300 tar -C /mnt/app-backup/uploads --no-wildcards --anchored --exclude-from="$stage/excluded-unit-photos.txt" -czf "$stage/operational-uploads.tgz" .
resume
...
mkdir -p /mnt/app-backup/backups/scheduled
target=$(mktemp -d "/mnt/app-backup/backups/scheduled/.partial-$stamp.XXXXXX")
cp -- "$stage/"* "$target/"
(cd "$target" && sha256sum -c SHA256SUMS)
sync -f "$target/database.dump"
sync -f "$target/operational-uploads.tgz"
mv -T -- "$target" "/mnt/app-backup/backups/scheduled/$stamp"
rm -- "$stage/"*
rmdir -- "$stage"
echo "Verified NAS backup complete: /mnt/app-backup/backups/scheduled/$stamp"
