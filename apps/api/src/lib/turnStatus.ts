...
export type TurnStages = { makeReadyStatus?: string | null; completionStatus?: string | null; paintStatus?: string | null; cleaningStatus?: string | null };
const status = (value?: string | null) => String(value ?? "").trim().toUpperCase();
...
export const tradeDone = (value?: string | null) => ["DONE", "COMPLETE", "COMPLETED", "NOT NEEDED", "N/A"].includes(status(value));
export function awaitingFinalWalk(item: TurnStages) {
  ...
  return isFinalWalkStatus(item.makeReadyStatus) || (!turnApproved(item) && repairsDone(item) && tradeDone(item.paintStatus) && tradeDone(item.cleaningStatus));
}
