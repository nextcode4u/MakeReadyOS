...
export const onCallSchema = z.object({
  ...
  rotation: z.object({ enabled: z.boolean(), startDate: dateOnly, weekday: z.number().int().min(0).max(6), at: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/), personIds: z.array(id).min(1).max(200), propertyIds: z.array(id).min(1).max(100) }).strict().nullable().optional(),
