...
import { turnDefinitions, turnSetupPrefix, turnStages } from "./turnSetup.js";
...
export async function ensureDefaultTurnScheduling(tx: Prisma.TransactionClient, propertyId: string) {
  ...
  await tx.operatingCalendar.upsert({ where: { propertyId }, create: { propertyId, noWeekendScheduling: true }, update: { noWeekendScheduling: true } });
  for (const [index, definition] of turnDefinitions(propertyId, [1, 1, 1, 1, 1], fields).entries()) {
