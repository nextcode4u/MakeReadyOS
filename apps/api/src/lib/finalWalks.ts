...
import { awaitingFinalWalk, turnApproved } from "./turnStatus.js";
...
export async function syncFinalWalks(propertyId: string, itemId?: string) {
  return prisma.$transaction(async db => {
    ...
    for (const item of items) {
      const ready = awaitingFinalWalk(item);
      const closed = item.isArchived || turnApproved(item);
      ...
      if (closed || !ready) {
        await db.workAssignmentBlock.updateMany({ where: { id: { in: blocks.map(block => block.id) } }, data: { status: turnApproved(item) ? "DONE" : "CANCELED" } });
        continue;
      }
      let block = blocks[0];
