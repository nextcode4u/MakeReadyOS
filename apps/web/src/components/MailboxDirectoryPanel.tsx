...
[I will paste the directory below or attach its file/image.]`;
  const update = (change: () => void) => { change(); setPlan(null); setSkipInvalid(false); setError(""); };
  async function submit(apply: boolean) {
    setBusy(true); setError("");
    try {
      if (!apply) setSkipInvalid(false);
      const result = await importMailboxDirectory(propertyId, { mode, text, overwrite, skipInvalid: apply && skipInvalid, ...(apply && plan ? { token: plan.token } : {}) });
  ...
  return <details data-testid="mailbox-directory-panel" style={{ gridColumn: "1 / -1", minWidth: 0 }}><summary>Mailbox directory / import</summary>
    ...
    <p>Import any subset of units; a complete directory is not required. Mailbox assignments populate final-walk drafts automatically. Imports update existing active units only, never other properties or availability.</p>
    ...
    <fieldset disabled={busy || !property} style={{ minWidth: 0 }}>
      ...
      {mode === "DIRECTORY" ? <><p>Paste CSV or tab-separated columns <code>unit,mailbox</code>. Numeric units match with or without leading zeros (11 matches 011) when unique in this property. Saved unit numbers and mailbox formatting are preserved. Blank mailbox cells are skipped, not cleared.</p><label>Directory file<input type="file" accept=".csv,.tsv,.txt" onChange={event => {
      ...
      {plan ? <div><p role="status">{plan.applied ? "Saved" : "Preview"}: {plan.changes.filter(change => change.action === "UPDATE").length} mailbox assignments {plan.applied ? "updated" : "to update"} for {property?.code}.</p>
        {plan.errors.length ? <div><p>{plan.applied ? `${plan.errors.length} rows were skipped; their assignments were not changed.` : `${plan.errors.length} rows cannot be imported. Fix them or skip them below.`}</p><ul>{plan.errors.map((error, i) => <li key={i}>{error}</li>)}</ul>{!plan.applied ? <label style={{ display: "flex", alignItems: "center", gap: 8 }}><input style={{ width: "auto" }} type="checkbox" checked={skipInvalid} onChange={event => setSkipInvalid(event.target.checked)}/>Skip flagged rows and import valid rows only</label> : null}</div> : null}
        ...
        {!plan.applied ? <button type="button" className="button button-primary" disabled={(!!plan.errors.length && !skipInvalid) || !plan.changes.some(change => change.action === "UPDATE")} onClick={() => void submit(true)}>Apply mailbox import to {property?.code}</button> : null}
      </div> : null}
    </fieldset>
