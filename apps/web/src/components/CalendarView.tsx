...
function trackGuidance(track: ScheduleTrack, events: CalendarEvent[]) {
  ...
  const riskCues = [
    track.overdueEnabled ? "overdue" : null,
    track.moveInSoonEnabled ? "move-in soon" : null,
  ].filter(Boolean).join(" and ") || "manual review";
  const warnings = [
    weekendCount ? `${weekendCount} weekend item${weekendCount === 1 ? "" : "s"}` : null,
    crowdedDays ? `${crowdedDays} crowded day${crowdedDays === 1 ? "" : "s"} with 3+ items` : null,
    highRiskCount ? `${highRiskCount} high-risk item${highRiskCount === 1 ? "" : "s"}` : null,
  ].filter(Boolean);
...
function dayConflictBadges(day: Date, events: CalendarEvent[]) {
  if (!events.length) return [];
  const dayOfWeek = day.getDay();
  return [
    dayOfWeek === 0 || dayOfWeek === 6 ? "Weekend" : null,
    dayOfWeek === 1 || dayOfWeek === 5 ? "Mon/Fri" : null,
    events.length >= 3 ? `${events.length} scheduled` : null,
    events.some((event) => event.riskLevel === "HIGH" || event.riskLevel === "CRITICAL") ? "High risk" : null,
    events.some((event) => event.overdue) ? "Overdue" : null,
...
function CalendarPanel({ track, events, labelsByField, month, onMonthChange, index, options, onTrackChange, onOpenItem }: {
  ...
  const guidance = useMemo(() => track ? trackGuidance(track, events) : null, [track, events]);
      ...
      <div className="calendar-legend" data-testid={`calendar-legend-${index}`}>
        ...
        {guidance ? (
          <div className={guidance.warnings.length ? "calendar-track-guidance warning" : "calendar-track-guidance"} data-testid={`calendar-track-guidance-${index}`}>
            <span><strong>Risk cues:</strong> {guidance.riskCues}</span>
            <span><strong>Compatibility:</strong> {guidance.warnings.length ? guidance.warnings.join(" / ") : "No weekend, crowded-day, or high-risk conflicts in the current filtered month."}</span>
          </div>
        ) : null}
      </div>
      <div className="calendar-grid">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => <div className="calendar-dow" key={day}>{day}</div>)}
        {grid.map((day) => {
          ...
          const dayState = isToday ? "Today" : isPast ? "Past day" : "Upcoming day";
          const conflictBadges = dayConflictBadges(day, dayEvents);
          return (
            <div
              className={classes}
              key={key}
              data-testid={isToday ? "calendar-today" : isPast ? "calendar-past-day" : undefined}
              aria-label={`${dayState}, ${day.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" })}`}
            >
              <div className="calendar-date">{isToday ? <span>Today</span> : null}{day.getDate()}</div>
              {conflictBadges.length ? (
                <div className="calendar-day-conflicts" data-testid={`calendar-day-conflicts-${key}`} aria-label={`Schedule warnings for ${key}: ${conflictBadges.join(", ")}`}>
