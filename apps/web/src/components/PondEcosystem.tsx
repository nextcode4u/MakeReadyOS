...
import { gardenWaterings, localPondDate, pondAntics, pondSeason, pondSecrets, pondWildlife, wildlifeVisible, type WildlifeId } from "../lib/pondDiscoveries";
...
export function PondPixel({ kind, silhouette = false }: { kind: string; silhouette?: boolean }) {
  if (kind === "fireflies") return <svg viewBox="0 0 20 20" shapeRendering="crispEdges" aria-hidden="true" className={`pond-pixel pond-pixel-fireflies${silhouette ? " pond-undiscovered" : ""}`}>{[[3,3],[5,3],[13,3],[15,3],[3,5],[15,5],[1,7],[17,7],[1,9],[17,9],[3,11],[15,11],[5,13],[7,13],[9,13],[11,13],[13,13],[3,15],[15,15]].map(([x,y],i) => <rect key={i} x={x} y={y} width="1" height="1" fill="#e0f892" style={{ "--scatter-x": `${(i % 5 - 2) * 4}px`, "--scatter-y": `${(i % 7 - 3) * 3}px` } as CSSProperties}/>)}</svg>;
  const paths = motifs[kind] ?? motifs.flower;
  return <svg viewBox="0 0 20 20" shapeRendering="crispEdges" aria-hidden="true" className={`pond-pixel pond-pixel-${kind}${silhouette ? " pond-undiscovered" : ""}`}><path d={paths[0]} fill="var(--wild-color, #9dcc8a)"/><path d={paths[1]} fill="var(--wild-detail, #244b48)"/></svg>;
}
...
export function PondWildlife(props: Props) {
  ...
  return <>
    ...
    {visible.map((entry, index) => <button key={entry.id} type="button" data-testid={`pond-wild-${entry.id}`} className={`pond-wildlife pond-wild-${entry.id}`} aria-label={`Observe ${entry.name}`} onFocus={() => setFocused(entry.id)} onBlur={() => setFocused(null)} onClick={() => { record(`wild-${entry.id}`); setMessage(entry.id === "snail" ? "Taking my time. The snail approves of a little break." : `${entry.name}: ${entry.hint}`); sound("bubble"); }} style={{ "--wild-index": index } as CSSProperties}><PondPixel kind={entry.id}/><span>{entry.name}</span>{entry.id === "strider" || entry.id === "duck" ? <i className="pond-wild-wake"/> : null}</button>)}
    <button type="button" className={`pond-secret${liveEffect ? " pond-secret-active" : ""}`} data-testid="pond-theme-secret" aria-label={secret.action} onClick={activate}><PondPixel kind={theme === "pond-15" && gardenWaterings(discovered) < 3 ? "flower" : secret.icon}/><span>{secret.action}</span></button>
    ...
    {liveEffect ? <div className={`pond-secret-effect pond-secret-effect-${theme}`} aria-hidden="true">{theme === "pond-10" ? <PondPixel kind="ghost"/> : Array.from({ length: 5 }, (_, i) => <i key={i} style={{ "--spark": i } as CSSProperties}/>)}</div> : null}
...
export function PondFieldGuide({ discovered }: { discovered: Record<string, string> }) {
  ...
  return <details className="pond-field-guide" data-testid="pond-field-guide"><summary>Field guide <span>{journal.length} / {entries.length} discoveries</span></summary><p>Wildlife sightings are recorded automatically while the pond plays. Secrets have no time limit. Saved in this browser, not synced between devices.</p><div className="pond-guide-entries">{entries.map(entry => <div key={entry.id} data-discovery={entry.id}><PondPixel kind={entry.icon} silhouette={!discovered[entry.id]}/><strong>{discovered[entry.id] ? entry.name : "Undiscovered"}</strong><small>{entry.hint}</small></div>)}</div><h3>What was that?</h3>{journal.length ? <ul>{journal.map(entry => <li key={entry.id}><strong>{entry.name}</strong> <time dateTime={discovered[entry.id]}>{new Date(discovered[entry.id]).toLocaleDateString()}</time></li>)}</ul> : <p>Stay a little while. Your first sighting will appear here.</p>}</details>;
}
