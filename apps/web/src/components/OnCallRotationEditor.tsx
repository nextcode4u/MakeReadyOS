...
export function OnCallRotationEditor({ data, schedule, change }: { data: OnCallData; schedule?: OnCallSchedule; change: (patch: Partial<OnCallData>) => void }) {
  ...
  const patch = (values: Partial<OnCallRotation>) => {
    if (!rotation) return;
    const next = { ...rotation, ...values };
    if (next.startPersonId && !next.personIds.includes(next.startPersonId)) next.startPersonId = next.personIds[0];
    change({ rotation: next });
  };
  ...
  return <details open><summary>Automatic weekly rotation</summary>
    ...
    {!rotation ? <button type="button" disabled={!data.people.length || !data.properties.length} onClick={() => change({ rotation: { enabled: true, startDate: latestFriday(data.timeZone), weekday: 5, at: "17:00", personIds: data.people.map(person => person.id), propertyIds: data.properties.map(property => property.id) } })}>Set up weekly rotation</button> : <>
      ...
      <label>Start rotation with<select required value={rotation.startPersonId ?? rotation.personIds[0] ?? ""} onChange={event => patch({ startPersonId: event.target.value })}>{!rotation.personIds.length && <option value="">Add a person to the rotation</option>}{rotation.personIds.map(id => <option key={id} value={id}>{label(id)}</option>)}</select></label>
      <p>The selected person starts on the first selected weekday on or after the start date, then coverage follows the order below and wraps around. To start with the person currently on call, use the date their current week began. Each person covers one week. Times follow {data.timeZone}, including daylight-saving changes.</p>
      {rotation.enabled && data.shifts.some(shift => Date.parse(shift.end) > Date.now() && shift.propertyIds.some(id => rotation.propertyIds.includes(id))) && <p role="status"><strong>Manual shifts override this rotation.</strong> Review the manual shifts below if the calendar shows a different person or handoff time. Changing the starting person does not remove those overrides.</p>}
      ...
      <h3>Swaps &amp; emergency coverage</h3><p>Changes cover all rotation properties for the specified period, then the regular rotation resumes. Reasons are visible to staff and authorized editing-code coordinators, never to public or viewing-code readers.</p>
      {changes.map(entry => {
