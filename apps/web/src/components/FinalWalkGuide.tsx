...
function InspectorEditor({ propertyId, settings, onDraftChange }: { propertyId: string; settings: FinalWalkSettings; onDraftChange: (state: { dirty: boolean; busy: boolean }) => void }) {
...
export function FinalWalkGuide({ properties }: { properties: Property[] }) {
  ...
  return <section className="turn-setup span-full" data-testid="final-walk-guide"><h2>Who does the final walk?</h2><p>Give final inspections their own owner and a clear backup, with assignments in My Work and in-app notifications when the unit is ready.</p><label>Final walks for<select value={propertyId} disabled={draft.busy} onChange={event => {
    ...
    {draft.dirty ? <p role="status">Unsaved inspector order. Save before leaving this setup.</p> : null}
    ...
    {query.data ? <InspectorEditor key={propertyId} propertyId={propertyId} settings={query.data} onDraftChange={setDraft} /> : null}
  </section>;
}
