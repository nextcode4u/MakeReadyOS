...
export function OnCallPanel({ external = false, userId = "" }: { external?: boolean; userId?: string }) {
  ...
  async function reload() {
    ...
    try {
      ...
      if (!external && !next.canEdit) { setDraft(null); setEditing(false); setNewCode(""); setRevoke(false); }
    } catch (cause) {
      if (request !== generation.current) return;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setNow(Date.now());
      setSaved(value => value ? { ...value, unlocked: false, expiresAt: null, data: { ...value.data, properties: value.data.properties.map(({ id, name }) => ({ id, name, address: "", shopLocation: "", accessCodes: "", instructions: "", mapUrl: "", guideUrl: "" })) } } : null);
    }, Math.max(0, saved.expiresAt - Date.now()));
    return () => clearTimeout(timer);
  }, [external, saved?.expiresAt]);
  useEffect(() => {
    if (!draft && !newCode && !revoke) return;
    const warn = (event: BeforeUnloadEvent) => { event.preventDefault(); event.returnValue = ""; };
    window.addEventListener("beforeunload", warn);
    return () => window.removeEventListener("beforeunload", warn);
  }, [draft, newCode, revoke]);
  ...
  return <section className={`on-call ${external ? "on-call-external" : ""}`} data-testid="on-call-panel">
    <header className="on-call-header"><div><span className="eyebrow">MakeReadyOS / On-call</span><h1>{data?.title ?? "On-call"}</h1><p>{external ? "Shared schedule. Property access instructions require the access code." : "Shared coverage directory, independent of property filters and regular user accounts."}</p></div>
      <div className="on-call-actions">
        ...
        {!external && saved?.canEdit ? <button type="button" disabled={busy} onClick={() => { if (editing && (draft || newCode || revoke) && !window.confirm("Discard unsaved on-call changes?")) return; setDraft(null); setNewCode(""); setRevoke(false); setEditing(!editing); }}> {editing ? "Close editor" : "Manage on-call"}</button> : null}
    ...
    {data ? <>
      {editing && !external && current?.canEdit ? <form className="on-call-editor" onSubmit={async event => {
        event.preventDefault(); if (busy || !current) return;
        setBusy(true); setError(""); setMessage(""); generation.current++;
        try { const result = await saveOnCall(userId, { version: current.version, data: current.data, externalEnabled: current.externalEnabled, ...(newCode ? { accessCode: newCode } : {}), revokeAccess: revoke }); generation.current++; setSaved(result); setDraft(null); setNewCode(""); setRevoke(false); setMessage("On-call saved. External viewers can only read the shared module."); }
        ...
        <fieldset disabled={busy}>
          ...
          <label className="on-call-check"><input type="checkbox" checked={current.externalEnabled} onChange={event => setDraft({ ...current, externalEnabled: event.target.checked })}/>Enable external sharing: names, phone numbers, property names and shift notes are public</label>
          ...
          <label className="on-call-check"><input type="checkbox" checked={revoke} onChange={event => setRevoke(event.target.checked)}/>Revoke all external guide sessions on save</label>
          ...
          <div className="on-call-actions"><button className="button button-primary" type="submit" disabled={!draft && !newCode && !revoke}>{busy ? "Saving..." : "Save on-call"}</button><button type="button" onClick={async () => { if ((draft || newCode || revoke) && !window.confirm("Discard unsaved changes and reload the saved workspace?")) return; setDraft(null); setNewCode(""); setRevoke(false); await reload(); }}>Reload saved workspace</button>{draft || newCode || revoke ? <span role="status">Unsaved changes</span> : null}</div>
        </fieldset>
      </form> : <>
      ...
      <footer className="helper-copy">{saved?.updatedAt ? `Last saved ${date(saved.updatedAt)}. ` : "Not configured yet. "} {external ? "Read-only shared module. Changes must be made by your coordinator." : "Only managers and admins can edit. Property access instructions are shared with all signed-in staff."}</footer>
    </> : null}
  </section>;
}
