...
export function PondTeamMilestones({ propertyId, viewerId, language }: { propertyId: string; viewerId: string; language: UserLanguage }) {
  ...
  return <details className="frog-settings pond-team-milestones" data-testid="pond-team-milestones" open={Boolean(propertyId)}>
    ...
    <p className="muted">{spanish ? "Cada turno aprobado cuenta una vez para todo el equipo: técnicos, pintores, limpieza y arrendamiento. Sin clasificaciones ni carreras." : "Each approved turn counts once for the whole team: technicians, painters, cleaners, and leasing. No rankings or races."}</p>
    {query.isPending ? <p role="status">{spanish ? "Cargando logros..." : "Loading milestones..."}</p> : query.isError ? <p role="status">{spanish ? "No se pudieron actualizar los logros." : "Milestones could not be refreshed."} <button type="button" className="button button-secondary" onClick={() => void query.refetch()}>{spanish ? "Reintentar" : "Retry"}</button></p> : <>
      ...
      {query.data.properties.map(property => {
        ...
        return <section key={property.id} className="pond-team-progress" data-testid={`pond-team-${property.id}`} aria-label={property.name}>
          ...
          {latest ? <strong className="pond-team-earned">{spanish ? "Logro obtenido" : "Milestone earned"}: {turnText(language, latest.name)}</strong> : <span>{spanish ? "El primer logro llega con el primer turno aprobado." : "Your first milestone starts with one approved turn."}</span>}
          {next ? <><progress value={property.completedTurns} max={next.goal} aria-label={`${property.name}: ${turnText(language, next.name)}`} /><small>{spanish ? "Siguiente" : "Next"}: {turnText(language, next.name)} / {property.completedTurns} {spanish ? "de" : "of"} {next.goal}</small></> : <small>{spanish ? "Todos los logros obtenidos. Cada nuevo turno sigue contando." : "All milestones earned. Every new approved turn still counts."}</small>}
          <details><summary>{spanish ? "Ver todos los logros" : "View all milestones"}</summary><ul>{query.data.milestones.map(entry => <li key={entry.goal}>{entry.goal <= property.completedTurns ? (spanish ? "Obtenido" : "Earned") : (spanish ? "Próximo" : "Upcoming")} / {entry.goal}: {turnText(language, entry.name)}</li>)}</ul></details>
        </section>;
      })}
    </>}
    <small className="muted">{spanish ? "Cuenta nuevas aprobaciones desde que se activó esta función, no unidades importadas ni anulaciones administrativas. Reabrir no agrega credito. Los estilos y descubrimientos siguen siendo personales." : "Tracks new approvals since this feature was enabled, not imported ready units or administrative overrides. Reopening never adds credit. Outfits and discoveries remain personal."}</small>
  </details>;
}
