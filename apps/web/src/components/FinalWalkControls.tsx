...
function FinalWalkContent({ itemId, propertyId, propertyName, currentUser, onMarkReady }: Props) {
  ...
  return <div data-testid="final-walk-controls">
    ...
    {query.data?.unitReady ? <p data-testid="final-walk-recorded-ready">Unit already recorded ready. Inspection records are separate; ready status alone does not confirm that a final walk was performed.</p> : block ? <><p><strong>Final walk: {block.assignedUser.fullName}</strong> / {block.plannedDate.slice(0, 10)}</p><p>{query.data?.ready ? "Ready for inspection" : "Scheduled; waiting for the turn to be marked complete."}</p>
