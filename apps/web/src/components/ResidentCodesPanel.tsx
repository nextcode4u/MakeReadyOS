  ...
  return <section className="drawer-section" data-testid="resident-codes-panel">
    ...
    {current ? <form onSubmit={async event => {
      ...
      <fieldset className="turn-material-fields" disabled={busy || query.data?.readOnly}>
        {current.technicianFollowUp ? <section style={{ gridColumn: "1 / -1" }}><h4>{current.correctionPending ? "Final-walk corrections needed" : "Previous final-walk feedback"}</h4><p style={{ whiteSpace: "pre-wrap" }}>{current.technicianFollowUp}</p><label>Technician resolution<textarea maxLength={1000} rows={3} value={current.value.technicianResolution} onChange={event => change({ technicianResolution: event.target.value })} /></label><p>Record what you corrected and save, then mark your repairs Done to request another final walk. Internal notes do not print on the resident report.</p></section> : null}
        ...
        <h4 style={{ gridColumn: "1 / -1" }}>Resident door &amp; access codes</h4>
