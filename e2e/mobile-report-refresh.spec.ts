...
test("report editor retains unsaved inspection input during reconnect refresh", async ({ page }) => {
  ...
  const followUp = editor.getByRole("textbox", { name: "Resident-facing follow-up", exact: true });
  ...
  expect(saved.draft.value.followUp).not.toBe("Unsent inspection observation");
