...
test("unit access directory imports safely, syncs turn codes and restricts painter permissions", async ({ page, browser }, testInfo) => {
  ...
  const unitSearch = panel.getByPlaceholder("Search and select a unit...");
  await unitSearch.fill("no-such-unit");
  await expect(panel.getByText("No matching units", { exact: true })).toBeVisible();
  await expect(panel.getByRole("button", { name: "Reveal unit codes", exact: true })).toBeDisabled();
  await unitSearch.fill("011");
  await panel.getByRole("button", { name: "011 / codes recorded", exact: true }).click();
  ...
  await unitSearch.fill("01");
  await expect(panel.getByLabel("Door code", { exact: true })).toHaveCount(0);
  await expect(panel.getByRole("button", { name: "Reveal unit codes", exact: true })).toBeDisabled();
  await unitSearch.press("Enter");
  await expect(panel.getByRole("button", { name: "Reveal unit codes", exact: true })).toBeEnabled();
  await panel.getByRole("button", { name: "Clear selected unit", exact: true }).click();
  await expect(unitSearch).toHaveValue("");
  await expect(panel.getByRole("button", { name: "Reveal unit codes", exact: true })).toBeDisabled();
  await unitSearch.fill("011");
  await unitSearch.press("Enter");
  await panel.getByRole("button", { name: "Reveal unit codes", exact: true }).click();
