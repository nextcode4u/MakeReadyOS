...
test("mobile header omits setup checklist while Setup remains reachable through Views", async ({ page }) => {
...
test.describe("mobile workflow coverage", () => {
  ...
  test("mobile module workflows cover quick-create paths across pool, pest, lease, projects, PM, wiki, and maps", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Recent Issues", exact: true }).locator("..").getByText(leaseArea, { exact: false })).toBeVisible();
    await assertNoPageHorizontalOverflow(page);
