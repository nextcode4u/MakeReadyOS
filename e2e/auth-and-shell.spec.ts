...
test("final walks assign only when ready, appear in My Work and hand off safely", async ({ page, browser }, testInfo) => {
  ...
  expect((await page.request.patch(`${origin}/api/mailboxes/${property.id}/${unit.id}`, { headers, data: { mailboxNumber: "DIR-17", expected: null } })).ok()).toBeTruthy();
  ...
  try {
    ...
    await expect(codes.getByLabel("Mailbox number", { exact: true })).toHaveValue("DIR-17");
    ...
    await codes.getByLabel("Mailbox number", { exact: true }).fill("MB-42");
    ...
    expect(savedReport.draft.value.mailbox).toBe("MB-42");
    expect(savedReport.draft.value.mailboxKeys).toBe("2");
    const directory = await (await page.request.get(`${origin}/api/mailboxes/${property.id}`)).json();
    expect(directory.units.find((entry: any) => entry.id === unit.id).mailboxNumber).toBe("DIR-17");
  ...
  try {
    for (const user of users) {
      ...
      if (user.id === users[0].id) {
        ...
        await expect(preview.locator("body")).toContainText("MB-42");
        ...
        expect(execFileSync("pdftotext", [testInfo.outputPath("leasing-inspection-draft.pdf"), "-"], { encoding: "utf8" })).toContain("MB-42");
        expect((bytes.toString("latin1").match(/\/Type\s*\/Page\b/g) ?? []).length).toBe(1);
        await expect.poll(() => staffPage.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBeTruthy();
